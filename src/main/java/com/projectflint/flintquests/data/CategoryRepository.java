...
public final class CategoryRepository {
	...
	public static synchronized void load() {
		...
		if (ConfigManager.isPlayerBundle()) {
			for (String resource : ConfigManager.bundledCategoryResources()) loadBundledResource(resource);
			return;
		}
	...
	private static void loadBundledResource(String resource) {
		try (InputStream stream = ConfigManager.openBundledResource(resource)) {
			if (stream == null) throw new IOException("Missing bundled category resource " + resource);
			...
			throw new IllegalStateException("Failed to read bundled category: " + resource, exception);
		}
	}
