...
## Editor canvas interaction rules (v0.1.9-a+)
...
Standalone Flint Quests releases identify **ImKas** as the developer and use the **Flint Quests Custom License**. The license explicitly allows decompiling/reading the code, using the public API, and bundling/shading the unmodified official `.jar` in another mod distribution. It forbids copying, modifying, or re-uploading Flint Quests source code or assets as a standalone project under another name. Do not silently replace this license with MIT, generic ARR, or another license.
...
## v0.1.14-a permanent UI/editor rules

- Quest description/lore editing must use a genuine multiline control. **Enter means new line**; do not regress this field back to a single-line `EditBox`.
- Node hover lore must be built as multiple tooltip lines, not one `Component.literal(...)` with separators. The intended order is: quest title, description/lore lines, then editor-only controls if developer mode is enabled.
- Literal legacy `\n` sequences should continue to be accepted as line breaks for backwards compatibility.
- Group Header categories must always keep their expand/collapse control separate from the label row. Changing a category between Quest Page and Group Header must never erase the dropdown affordance when children exist.
- Existing categories must be deletable from the category editor. Category deletion must preserve the category tree by reparenting children and must move assigned quests to a valid Quest Page rather than orphaning them.


## v0.1.17-a permanent UI/text rules

- Author-entered player-facing text supports legacy Minecraft formatting codes with either `&` or `§` prefixes. Supported codes are `0-9`, `a-f`, `k-o`, and `r`. Raw codes remain visible in editor inputs; only player-facing rendering interprets them.
- Quest-node movement must not rebuild/replace the entire screen widget tree during an active drag. Node views may be recomputed, but the drag capture itself must remain stable until mouse release.
- Developer settings must expose both **Build Nestable Quest-Pack .jar** and **Open Built Jars** while Developer Environment is enabled.
- `flintquests-exports/` is the canonical output directory for player quest-pack jars.


## Quest-detail pagination and progress cleanup (v0.1.18-a)

Permanent UI/data rules from this release:

- Player quest detail pages must never silently clip text or tasks. Description lines, task lines, blank spacing, and interactive CHECKMARK controls are paginated together using one vertical content flow.
- CHECKMARK widgets must derive their screen position from that same flow; never maintain a separate hard-coded task Y counter.
- When a quest definition is deleted, its entire saved per-player progress entry is stale data and must be removed. This includes quest completion, completion timestamp, task values, and task-completion booleans.
- When a task is removed from a still-existing quest, stale progress for that task ID must also be removed.
- Diamond nodes must have an actual diamond-shaped body/border rather than only approximating the shape with a square hit/render frame.


## Themes and collaborative quest-data exchange (v0.1.19-a)

### Node geometry rule

Quest nodes now use an odd `31px` logical node size. This is intentional: shapes with a true center vertex, especially `DIAMOND`, must share one exact integer center for the body, normal border, glow border, icon placement, and dependency anchor. Do not revert Diamond rendering to an even-sized inset square approximation.

### Custom themes

- Theme JSON lives under `config/flintquests/themes/`.
- `default.json` is generated automatically.
- `flintquests.json` stores the active theme filename as `activeTheme`.
- Theme colors accept `#RRGGBB` or `#AARRGGBB`.
- Settings exposes **Theme: ...** and **Open Theme Folder**.
- Theme support is data-driven; authors should not need to modify Flint Quests source to recolor the quest UI.
- Nestable player quest-pack JARs embed the currently active theme so distributed quests retain the author's presentation.
- `docs/THEMES.md` is the field-level theme reference.

### Editable quest-data ZIPs

The nestable JAR exporter is for player distribution. Editable ZIP export/import is a separate collaboration workflow for authors and helpers.

- **Export Quest Data ZIP** writes editable project data to `flintquests-exports/quest-data/`.
- **Import Quest Data ZIP** opens a file picker and imports that data.
- **Open Quest Data ZIP Folder** opens the collaboration export directory.
- ZIPs contain quests, categories, quest-ID migrations, and the active theme.
- Player/world quest progress is never included.
- Import creates a full pre-import backup ZIP first.
- Imported quest/category definitions replace the editable local definition set; the included theme is merged and selected when available.
- Import rejects unknown manifests and validates ZIP extraction paths.
- `docs/QUEST_DATA_ZIP.md` is the persistent format/workflow reference.


## Canvas zoom and GUI-level editor toggle (v0.1.20-a)

Permanent behavior rules:

- Holding Control while scrolling the quest canvas zooms it rather than panning it.
- Scroll up zooms in; scroll down zooms out.
- Zoom is cursor-centered and must not cause the hovered graph position to jump away from the mouse.
- Zoom affects node spacing, node size/shape, dependency paths, hitboxes, panning bounds, and editor drag calculations together. A partially zoomed UI must not have invisible full-size hitboxes or unscaled connector math.
- Quest-book zoom is a player UI preference stored in `flintquests.json`.
- The editing-mode toggle is a real configurable Flint Quests keybinding and must work both in normal gameplay and while Flint Quests screens are active.
- GUI screens cannot rely only on `KeyMapping#consumeClick()` because Minecraft's normal gameplay keybinding click path is suppressed while most screens are open; Flint Quests screens therefore handle the configured toggle action directly from their `KeyEvent`.
- Disabling editing from inside an editor returns the user to the normal quest book instead of leaving author-only screens accessible.

## v0.1.21-a data-import and built-in-theme rules

- **Import Quest Data ZIP** should start in the user's `Downloads` folder when it exists; the Flint Quests export folder is only a fallback.
- A current editable-data ZIP is not valid merely because it is a `.zip`. It must contain the Flint Quests marker `flintquests-data.json`, a supported `format`, the expected `type`, and for v2 the schema `flintquests.quest-data.v2` plus explicit `quests/` and `categories/` containers.
- Import validation must finish before a backup/replacement mutates the active quest project.
- Keep reasonable backwards compatibility for Flint Quests' own previous editable-data export formats when they can be positively identified.
- Flint Quests ships/generates several baseline JSON themes: Default, Light, Dark, AMOLED, Red, Crimson, Brown, and Vanilla Minecraft.
